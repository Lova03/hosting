...
function MinecraftProductsPage() {
  ...
  return (
    <div className='page flex flex-col isolate'>
      <Helmet>
        <title>Minecraft Server Hosting | ZoBlaze</title>
        <meta
          name='description'
          content='Explore top-tier Minecraft server hosting with ZoBlaze. Enjoy low latency, high performance, and exceptional support to keep your Minecraft community thriving.'
        />
      </Helmet>
      ...
      {hasError && (
        <div className='min-h-[calc(100vh-10rem)] w-full flex flex-col justify-center items-center text-center px-4'>
          ...
          <p className='text-white mb-6'>
            We're having trouble loading the Minecraft products right now. Please try again later.
          </p>
        ...
        <div className='flex w-full flex-col items-center mt-8'>
          ...
          <div className='relative w-full max-w-5xl mx-6'>
            ...
            <div
              ...
              {minecraft.map((item, idx) => (
                <MinecraftProductCard key={idx} product={item} />
              ))}
              {searchTerm.length > 0 &&
                discordBot.map((item, idx) => <DiscordProductCard key={idx} product={item} />)}
              {searchTerm.length > 0 &&
                vps.map((item, idx) => <VPSProductCard key={idx} product={item} />)}
            </div>
            {/* No Cards Found */}
...
export default MinecraftProductsPage;
