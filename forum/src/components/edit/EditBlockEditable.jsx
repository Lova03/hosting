...
function EditBlockEditable({ block: { type, id } }) {
  ...
  const removeSelf = () => {
    dispatch(removeBlockToEdit(id));
  };

  const handleMoveUp = () => {
    dispatch(moveBlockToEdit({ id, dir: 'up' }));
  };

  const handleMoveDown = () => {
    dispatch(moveBlockToEdit({ id, dir: 'down' }));
  };

  const handleChange = (value) => {
    if (type === 'code') dispatch(setContentToEdit({ id, content: value }));
    if (type === 'text') {
      dispatch(setContentToEdit({ id, content: value.target.value }));
      adjustHeight();
    }
  };
...
export default EditBlockEditable;
